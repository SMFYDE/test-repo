...
class TextractWrapper:
    ...
    def start_detection_job(self, bucket_name, document_file_name):
        """
        Starts an asynchronous job to detect text elements in an image stored in an
        Amazon S3 bucket. The image must be in PNG, JPG, or PDF format.
...
file_path = "OCR/test/01. Kbis.pdf"
...
print(f"textract_client: {textract_client}")
print(f"s3_resource: {s3_resource}")
print(f"sqs_resource: {sqs_resource}")

# Instanciation du wrapper
wrapper = TextractWrapper(textract_client, s3_resource, sqs_resource)

# 1. Uploader le document sur S3 (si ce n'est pas déjà fait)
with open(file_path, 'rb') as f:
    wrapper.prepare_job(bucket_name, file_path, f)

# 2. Démarrer le job asynchrone (sans SNS ni rôle IAM)
job_id = wrapper.start_detection_job(
    bucket_name,
    file_path
)

# 3. Attendre la complétion (sans poll SQS)
status = None
while status not in ("SUCCEEDED", "FAILED"):
    response = wrapper.get_detection_job(job_id)
    status = response["JobStatus"]
    print(f"Statut du job Textract : {status}")
    if status not in ("SUCCEEDED", "FAILED"):
        time.sleep(5)

# 3. Attendre la complétion (poll SQS)
# while status != 'SUCCEEDED':
#     status = wrapper.check_job_queue('https://sqs.region.amazonaws.com/account-id/queue-name', job_id)
#     if status is None:
#         time.sleep(5)  # Attendre avant de re-poller

# 4. Récupérer les résultats
result = wrapper.get_detection_job(job_id)
print(result)
